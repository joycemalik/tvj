...
content = content.replace('if proxy_score > best_proxy_w:', 'if wing == 20.0 and abs(center-1215.0) < 0.2:\n                    print(f"center={center:.2f} sigma={sigma:.2f} noise={noise:.2e} chi2={chi2_red:.2f}")\n                if proxy_score > best_proxy_w:')
...
noise = estimate_noise(sub_y / abs(np.mean(cont))) * abs(np.mean(cont))
print(f"OUTSIDE NOISE: {noise:.2e}")
